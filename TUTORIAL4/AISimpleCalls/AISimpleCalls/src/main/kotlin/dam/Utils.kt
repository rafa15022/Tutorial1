...
/**
 * Reads an optional Double from config.properties.
 * Example: temperature=0.2
 */
fun Properties.getOptionalDouble(name: String): Double? {
...
/**
 * Reads an optional Int from config.properties.
 * Example: max_tokens=300
 */
fun Properties.getOptionalInt(name: String): Int? {
    return getProperty(name)
        ?.trim()
        ?.takeIf { it.isNotEmpty() }
        ?.toIntOrNull()
...
fun configureLogging(properties: Properties) {
    ...
    try {
        ...
        loggerContext.getLogger("ROOT").level = logLevel
        loggerContext.getLogger("dam").level = logLevel
