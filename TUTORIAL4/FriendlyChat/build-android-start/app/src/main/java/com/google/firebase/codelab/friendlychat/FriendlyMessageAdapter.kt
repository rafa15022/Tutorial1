...
class FriendlyMessageAdapter(
    ...
    inner class MessageViewHolder(private val binding: MessageBinding) : ViewHolder(binding.root) {
        fun bind(item: FriendlyMessage) {
            ...
            // Set the text color based on the user
            ...
            if (item.photoUrl != null) {
                ...
                if (userName != null && userName != ANONYMOUS && currentUserName == userName) {
                    binding.messengerImageView.setImageResource(R.drawable.ic_account_circle_blue_36dp)
                } else {
    ...
    inner class ImageMessageViewHolder(private val binding: ImageMessageBinding) :
        ...
        fun bind(item: FriendlyMessage) {
            ...
            binding.messengerTextView.text = userName ?: ANONYMOUS
    ...
    private fun loadImageIntoView(view: ImageView, url: String, isCircular: Boolean = true) {
        // Versão sem Firebase Storage: se existir um URL normal, o Glide carrega diretamente.
        // As mensagens novas desta versão são apenas texto.
    ...
    override fun stopListening() {
        // 1. Capture how many items are currently in the snapshot list
        val oldSize = snapshots.size
        // 2. Detach the Firebase listener & clear the snapshots
        super.stopListening()
        // 3. Tell RecyclerView “positions 0…oldSize-1 were removed”
        if (oldSize > 0) {
